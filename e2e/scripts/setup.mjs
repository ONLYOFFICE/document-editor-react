...
if (libVersion) {
  console.log(`== Installing @onlyoffice/document-editor-react@${libVersion} from npm ==`);
  run('npm', ['install', `@onlyoffice/document-editor-react@${libVersion}`, '--no-save'], e2eRoot);
} else {
  console.log('== Building @onlyoffice/document-editor-react ==');
  run('npm', ['run', 'rollup'], repoRoot);
